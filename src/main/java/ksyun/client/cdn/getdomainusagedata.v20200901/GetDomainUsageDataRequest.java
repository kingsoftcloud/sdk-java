...
/**
* @Classname GetDomainUsageDataRequest
* @Description 请求参数
*/
@Data
public class GetDomainUsageDataRequest{
    ...
    /**需要获取的用量类型，一次只允许查询一种类型bandwidth：带宽数据traffic：流量数据static-https：静态https请求数dynamic-http：动态http请求数dynamic-https：动态https请求数static-quic：静态quic请求数dynamic-quic：动态quic请求数 <br /><br />PS：Metric取值为bandwidth或者traffic的时候areas生效，请求数不区分区域*/
    ...
    /**计费区域名称，只允许输入单区域查询。取值：中国大陆 - CN，亚太一区 - AP1，亚太二区 - AP2，亚太三区 - AP3，北美洲 - NA，欧洲 - EU，非洲 - AF，南美洲 - SA。<br /><br />PS：只有Metric取值为bandwidth或者traffic的时候生效，单次允许查询一个或多个区域，多个区域间逗号（半角）分隔，缺省为 CN；请求数不区分区域，当输入条件为请求数任意一种时，此处缺省为全部区域*/
    @KsYunField(name="Areas")
    private String Areas;

    /**统计粒度，取值为 5：5分钟粒度；60：1小时粒度；1440：1天粒度；缺省为5分钟粒度5分钟及以上粒度的带宽峰值均取该粒度时间段内的5分钟粒度的最高峰值点单次查询最多可获取最近 一年 内 31天 跨度的数据*/
