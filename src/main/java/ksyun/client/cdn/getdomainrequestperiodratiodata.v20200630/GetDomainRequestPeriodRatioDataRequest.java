...
/**
 * @Classname GetDomainRequestPeriodRatioDataRequest @Description 请求参数
 */
@Data
public class GetDomainRequestPeriodRatioDataRequest {
  ...
  /**
   * 获取数据起始时间点，不可与CurrentPeriodStartTime相同，日期格式按ISO8601表示法，北京时间，格式为：YYYY-MM-DDThh:mm+0800，例如：
   * 2016-08-01T21:14+0800
   */
  ...
  /**
   * 指定查询指标，单选，支持的类型为：**flow**：流量，单位为 byte；**bandwidth**：带宽，单位为 bps；**request**：请求数，单位为
   * 次；**qps**：平均每秒请求次数，单位：次
   */
  ...
  /**
   * 统计粒度，取值为 1：1分钟粒度；取值为
   * 5：5分钟粒度；60：1小时粒度；1440：1天粒度；5分钟及以上粒度的带宽值均取该粒度时间段内的5分钟粒度的峰值点。默认为5分钟粒度1分钟粒度查询范围最大跨度为24小时5分钟粒度查询范围最大跨度为31天其余粒度查询范围最大跨度为31天
   */
  ...
  /**
   * 省份区域名称，
   * 枚举类型表见[使用须知](https://docs.ksyun.com/documents/196#37)；支持省份批量查询，多个省份用逗号（半角）分隔，缺省为全部省份（当Areas选项有且仅有中国大陆选项时，此类型生效），具体Provinces见下附录说明
   */
  ...
  /**
   * 统计结果数据展示方式，支持不展开和按一种维度展开，缺省为
   * 不展开（unexpand）可展开维度包含：区域（area）、域名（domain）、省份（province）、运营商（isp）、IP类型（ip）、协议类型（schema）,
   */
