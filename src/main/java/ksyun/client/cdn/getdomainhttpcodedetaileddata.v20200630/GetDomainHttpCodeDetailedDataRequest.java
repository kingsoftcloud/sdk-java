...
/**
 * @Classname GetDomainHttpCodeDetailedDataRequest @Description 请求参数
 */
@Data
public class GetDomainHttpCodeDetailedDataRequest {
  ...
  /**
   * 统计粒度，取值为 1：1分钟粒度；取值为 5：5分钟粒度；60：1小时粒度；1440：1天粒度;<br>
   * 5分钟以上粒度的值取该粒度时间段内的5分钟粒度的累加和,缺省为5分钟粒度.<br>
   * 1分钟粒度查询范围最大跨度为24小时，5分钟粒度查询范围最大跨度为31天，其余粒度查询范围最大跨度为90天
   */
  ...
  /**
   * 省份区域名称，
   * 枚举类型表见【[使用须知](https://docs.ksyun.com/documents/196#37)】；支持省份批量查询，多个省份用逗号（半角）分隔，缺省为全部省份（当Areas选项有且仅有中国大陆选项时，此类型生效）
   */
  @KsYunField(name = "Provinces")
  private String Provinces;

  /**
   * 运营商名称，枚举类型表见【[使用须知](https://docs.ksyun.com/documents/196#38)】；只允许输入一种类型，缺省为全部运营商，（当Areas选项有且仅有中国大陆选项时，此类型生效）
   */
  ...
  /** 状态码分类，取值为：2xx/3xx/4xx/5xx，默认为全部状态码类型 */
