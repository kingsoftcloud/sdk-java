...
/**
* @Classname GetBandwidthDataRequest
* @Description 请求参数
*/
@Data
public class GetBandwidthDataRequest{
    ...
    /**S区域名称，枚举类型表见使用须知，支持多区域查询，多个区域用逗号（半角）分隔，缺省为 CN缺省为 CN*/
    ...
    /**统计粒度，取值为 5（默认）：5分钟粒度；10：10分钟粒度；20：20分钟粒度；60：1小时粒度；240：4小时粒度；480：8小时粒度；1440：1天粒度；以上粒度的带宽值均取该粒度时间段的带宽峰值，即每5分钟有效带宽值的最高带宽*/
    @KsYunField(name="Granularity")
    private Integer Granularity;

    /**数据类型，取值为edge：服务数据； origin：回源数据（push：直播推流加速暂不支持查询回源数据）； 支持多类型选择，多个类型用逗号（半角）分隔，缺省为 edge*/
    @KsYunField(name="DataType")
    private String DataType;

    /**协议类型， 取值为http：http协议数据；https：https协议数据；缺省默认返回全部协议数据（当CdnType=live时，请忽略此参数）*/
