...
/**
 * @Classname GetServerDataRequest @Description 请求参数
 */
@Data
public class GetServerDataRequest {
  ...
  /**
   * 指定查询指标，支持的类型有：**flow**：流量，单位为 byte；**bandwidth**：带宽，单位为
   * bps；**request**：请求数，单位为次；**qps**：平均每秒请求次数，单位：次，支持批量数据查询，多个类型用逗号（半角）分隔，默认为全部类型
   */
   ...
   * 不展开（unexpand）展开支持全部展开（expand）和按照某个查询维度展开，具体包含：区域（region）（回源不支持按照区域展开）、域名（domain）、协议类型（schema）
   */
