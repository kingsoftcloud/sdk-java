...
/**
 * @Classname UpdateCertificateRequest @Description 请求参数
 */
@Data
public class UpdateCertificateRequest {
  ...
  /** 验证方式（除Sectigo外仅针对DV证书，Sectigo全部需要） */
   ...
   *
   * <p>约束： - 当值为0时：
   * 可以用来修改Contact、CompanyName、State、City、Address、CompanyPhone、PostalCode、Department等信息； - 当值为1时： -
   * 必须保证Contact不为空 -
   * 且CompanyName、State、City、Address、CompanyPhone、PostalCode、Department不为空，或在IsSubmit为0时已经设置过 -
   * 对于DV证书，以上规则有两个例外 - Department可为空 - CompanyPhone可为空，但contact当中必须设置phone
   */
  ...
  /**
   * CSR字符串来源 - 约束 - IsSubmit为0时，此参数可缺省 - IsSubmit为1时 - 当值为USER时，参数CSR不可缺省或此前在IsSubmit为0时设置过； -
   ...
   * 证书签名算法。可选值
   *
   * <p>SHA256-FULLCHAIN SHA2-256 默认值
   */
