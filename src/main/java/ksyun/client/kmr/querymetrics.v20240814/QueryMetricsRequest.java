...
/**
 * @Classname QueryMetricsRequest @Description 请求参数
 */
@Data
public class QueryMetricsRequest {
  ...
  public static class QueryDataDto {
    ...
    /** 起始时间戳 为UNIX时间戳，单位为秒级，示例值1741050780 表示的是秒级时间戳（对应北京时间 2025-03-02 21:13:00) */
    @KsYunField(name = "Start")
    private String Start;

    /** 结束时间戳 为UNIX时间戳，单位为秒级，示例值 1741050780 表示的是秒级时间戳（对应北京时间 2025-03-04 09:13:00) */
    @KsYunField(name = "End")
    private String End;

    /**
     * 数据点之间的间隔，必须从范围查询中返回。 query 在 start 、 start+step 、 start+2*step 、…、 start+N*step 时间戳处执行。如果未设置
     * step，则默认为 5m （5分钟）
     */
