...
/**
 * @Classname StartRayJobRunRequest @Description 请求参数
 */
@Data
public class StartRayJobRunRequest {
  ...
  public static class RaySubmitDataDto {
    ...
    /** 用于配置作业的启动命令 格式：python + 文件名 */
    @KsYunField(name = "EntrypointCmd")
    private String EntrypointCmd;

    /** 作业的ks3文件路径 格式：ks3://xx/ray/ray-test.py */
    @KsYunField(name = "EntrypointResource")
    private String EntrypointResource;

    /** 作业自定义配置,多个参数用逗号分隔 格式： ["key1=value1", "key2=value2"] */
    @KsYunField(name = "Conf")
    private String Conf;

    /** 作业使用的镜像 如需使用自定义镜像，添加该配置，替换镜像地址 */
    @KsYunField(name = "Image")
    private String Image;

    /** JuiceFS配置 Ray head节点挂载 JuiceFS 存储卷 */
