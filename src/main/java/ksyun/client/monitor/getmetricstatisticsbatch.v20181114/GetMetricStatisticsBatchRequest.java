...
/**
 * @Classname GetMetricStatisticsBatchRequest @Description 请求参数
 */
@Data
public class GetMetricStatisticsBatchRequest {
   ...
   * <p>> **时间格式：** `2023-07-19T16:09:50Z`
   *
   * <p>> **注意：** 数据采集可能存在有2分钟左右的延迟，`EndTime`的值应当比当前时间延迟两分钟，如果当前时间为：2023-07-19T16:11:50Z，那么
   ...
   * 统计粒度，单位为秒。
   *
   * <p>> 注意：该参数值应为60的整数倍，值的大小可能会导致数据与控制台展示不一致。
   *
   * <p>> 默认值为空，将不对数据进行降采样，按产品线的推送频率进行展示。
   */
