...
public class CreateClusterRequest {
  /** 集群名称 有效值：2-64个字符，支持中文，英文，数字，以及特殊字符-_ */
  ...
  /** 集群部署方式 INDEPENDENT_CLUSTER:独立部署集群MANAGED_CLUSTER: 托管集群 */
  ...
  public static class NetworkDto {
    ...
    public static class VpcCNIDto {
      ...
      /** 开启vpc-cni必填 网络模式： • eniOnly 独占网卡 • eniMultiIP 共享网卡 */
      @KsYunField(name = "DaemonMode")
      private String DaemonMode;

      /** 开启vpc-cni后必填， vpc的子网id，必须选跟集群同vpc下的子网Id,可以填多个 */
      @KsYunField(name = "SubnetIds")
      private List<String> SubnetIdsList;

      /** 开启vpc-cni后必填， 弹性网卡安全组id，需要跟集群同vpc下的安全组id */
      @KsYunField(name = "SecurityGroup")
      private String SecurityGroup;
    }

    /** Pod CIDR 信息,如：10.0.0.0/16 • 不能与Service网段冲突 • 不能与VPC网段冲突 • 托管集群不支持33网段 */
    @KsYunField(name = "PodCidr")
    private String PodCidr;

    /** Service CIDR，如10.254.0.0/16 • 不能与Pod网段冲突 • 不能与Vpc网段冲突 */
    @KsYunField(name = "ServiceCidr")
    private String ServiceCidr;

    /** Pod数量上限/节点 默认值：128 可选值：16，32，64，128，256 */
  ...
  public static class AddonsDto {
    /**
     * 插件名称（会根据插件名称选择 • csi-driver • lb-controller （推荐默认选择该组件） • kube-proxy
     * （推荐默认选择该组件，确实该组件会影响集群网络功能） • nvidia-gpu （gpu机型才能选择）
     */
    private String Name;
  }
}
