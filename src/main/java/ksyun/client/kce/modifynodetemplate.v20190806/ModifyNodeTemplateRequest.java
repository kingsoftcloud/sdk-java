...
/**
 * @Classname ModifyNodeTemplateRequest @Description 请求参数
 */
@Data
public class ModifyNodeTemplateRequest {
  ...
  public static class NodeTemplateDto {
    /**
     * 计费方式，有效值： - Daily：按量付费（按日月结） - HourlyInstantSettlement：按量付费
    ...
    /** 实例名称 注：若不指定该参数，名称则为：AS-节点池名称-Worker */
    ...
    /**
     * 多子网扩展策略，有效值： - balanced-distribution：均衡分布 - choice-first：选择优先 默认值：balanced-distribution
     * 注：当绑定多个子网时，该项必填
     */
    ...
    /** 实例密码 校验规则：8-32个字符，必须包含大小写字母和数字，支持英文特殊字符!$%()*+,-./:;<=>?@[]^_`{ */
    @KsYunField(name = "Password")
    private String Password;

    /** 密钥id 校验规则：请填写有效的keyId，若填写此参数，则Password参数无效 注：参数KeyId.N和Password必须填写一个 */
    ...
    public static class AdvancedSettingDto {
      ...
      /**
       * 容器运行时，根据集群版本和需求选择 Kubernetes版本＜1.24时，有效值： - docker - containerd Kubernetes版本≥1.24时，有效值： -
       * contained 默认值：docker
       */
