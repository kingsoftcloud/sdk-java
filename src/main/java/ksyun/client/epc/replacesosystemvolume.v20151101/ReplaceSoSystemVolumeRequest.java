...
/**
 * @Classname ReplaceSoSystemVolumeRequest @Description 请求参数
 */
@Data
public class ReplaceSoSystemVolumeRequest {
  ...
  /**
   * 使用镜像预设密码/密钥对登录实例。取值： • true：使用镜像预设密码/密钥对登录。 • false（默认）：不使用镜像预设密码/密钥对登录。 说明 • 仅自定义镜像支持使用该功能。 •
  ...
  /**
   * 重置密钥对。 说明 • 仅对Linux操作系统的实例生效。 • KeepImageCredential为false时，本参数与Password不能同时为空。 •
