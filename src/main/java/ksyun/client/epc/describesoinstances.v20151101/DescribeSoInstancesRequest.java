...
/**
 * @Classname DescribeSoInstancesRequest @Description 请求参数
 */
@Data
public class DescribeSoInstancesRequest {
  /** 实例的计费方式，取值： • PostPaid：按量计费 • PrePaid：包年包月 示例值：PostPaid */
  @KsYunField(name = "InstanceChargeType")
  private String InstanceChargeType;

  /** 根据规格过滤实例，最多支持100个实例规格。 • 参数 - N：表示实例的序号。 undefined多个实例规格之间用&分隔 */
  ...
  /**
   * 实例的状态，取值： • CREATING：创建中 • RUNNING：运行中 • STOPPING：停止中 • STOPPED：已停止 • REBOOTING: 重启中 •
   * STARTING：启动中 • REBUILDING：重装中 • RESIZING：更配中 • ERROR：错误 • DELETING：删除中
   */
