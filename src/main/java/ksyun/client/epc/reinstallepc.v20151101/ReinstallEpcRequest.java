...
/**
 * @Classname ReinstallEpcRequest @Description 请求参数
 */
@Data
public class ReinstallEpcRequest {
  ...
  /**
   * 网卡模式 有效值： bond4：BOND模式 single：非BOND模式 dual：双网卡模式 1. centos、Debin、Ubuntu重装时，bond选项 限制： a.
   * 若开机为双网卡模式，则只能重装成双网卡模式，无法重装为bond和非bond模式。 b. bond/非bond重装时仅可选择bond/非bond 2.
   * 重装为window系统时，无法选择网卡类型，默认重装为非bond模式,且无法选择密钥验证。
   */
  @KsYunField(name = "NetworkInterfaceMode")
  private String NetworkInterfaceMode;

  /** 安全组件类型 - classic：经典版 - no：不开启 */
  ...
  /** RoCE存储卡名称，仅支持 eth8x_bond、storage_bond */
