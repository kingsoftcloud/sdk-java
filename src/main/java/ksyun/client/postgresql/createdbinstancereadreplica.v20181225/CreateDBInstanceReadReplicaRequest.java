...
/**
 * @Classname CreateDBInstanceReadReplicaRequest @Description 请求参数
 */
@Data
public class CreateDBInstanceReadReplicaRequest {
   ...
   * 实例名称
   *
   * <p>不超过256个字节，仅支持中文、大小写字母、数字、减号和下划线
   */
   ...
   * 计费方式
   *
   * <p>YEAR_MONTH（包年包月）,DAY（按日计费），默认值：YEAR_MONTH
   */
   ...
   * 购买时长
   *
   * <p>时长 默认值：1(单位:月) 注：billType=1(包年包月)则必填
   */
   ...
   * 可用区
   *
   * <p>示例(AvailabilityZone.1=cn-beijing-6a)，默认情况下只读实例可用区与主实例一致，如果主实例是跨可用区实例，只读实例会随机分布。
   */
