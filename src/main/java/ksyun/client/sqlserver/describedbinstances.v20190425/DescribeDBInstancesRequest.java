...
public class DescribeDBInstancesRequest {
   ...
   * <p>
   *
   * <p>```json 高可用版：HRDS_SS 高可用云盘版：SS_HRDS_E 临时版：TRDS_SS 临时云盘版：SS_TRDS_E ```
   */
   ...
   * 实例状态-运行中/请续费
   *
   * <p>```json 运行中：active 请续费：invalid 注意：此类型为订单计费的情况 注意：控制台执行：状态-运行中，状态-请续费 ```
   */
   ...
   * 实例状态-即将过期
   *
   * <p>```json 按照实例过期时间过滤，取值范围>0
   *
   * <p>注意：此参数一般用来概览实例计费情况 ```
   */
