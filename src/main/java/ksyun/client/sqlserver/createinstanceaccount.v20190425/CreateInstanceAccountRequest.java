...
/**
 * @Classname CreateInstanceAccountRequest @Description 请求参数
 */
@Data
public class CreateInstanceAccountRequest {
   ...
   * 账户名称
   *
   * <p>```json root, rdsrepladmin, rdsadmin, dtsroot,
   * sa不可用；由大小写字母、数字或下划线组成，以小写字母开头，以小写字母或数字结尾，长度为2~16个字符 ```
   */
   ...
   * 账户密码
   *
   * <p>```json 8-30个字符，必须包含大小写字母和数字；支持的特殊字符：!@#$%^&*()_+=- ```
   */
  ...
  public static class InstanceAccountPrivilegesDto {
     ...
     * 权限
     *
     * <p>```json DBOwner ReadWrite ReadOnly ```
     */
