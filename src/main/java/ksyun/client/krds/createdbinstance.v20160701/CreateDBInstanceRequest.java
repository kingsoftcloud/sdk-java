...
public class CreateDBInstanceRequest {
  ...
  /**
   * 计费方式 默认值：YEAR_MONTH ```json 包年包月:YEAR_MONTH 按量付费(按日月结):DAY 按量付费:HourlyInstantSettlement
   *
   * <p>备注： 1.按日月结（按日配置付费月结） 2.按量付费 (按小时实时结算，按量实时计费&结算） ```
   */
  ...
  /**
   * 可用区字段
   * 示例(AvailabilityZone.1=cn-beijing-6a&AvailabilityZone.2=cn-beijing-6b)，表示实例的主副本在a区，备副本在b区。如果没有跨可用区的需求，建议将实例创建在云主机的可用区内已减少网络延时。其中企业版ERDS可用区为3个（1主2备）
   */
