...
/**
 * @Classname SlowLogReportRequest @Description 请求参数
 */
@Data
public class SlowLogReportRequest {
  ...
  /**
   * queryCount/ 查询条数 queryTimeAvg/ 查询平均时间 queryTimeMax/ 查询最大时间 lockTimeAvg/ 平均等待时间 lockTimeMax/
   * 最大等待时间 rowsExaminedAvg/ 平均扫描行 rowsExaminedMax/ 最大扫描行 rowsSentAvg/ 平均返回行 rowsSentMax 最大返回行
   * processingTime 处理时间(不传排序字段和方式默认处理时间倒序)
   */
  @KsYunField(name = "SortBy")
  private String SortBy;

  /** 排序规则 ASC 正序(升序) DESC 倒序(倒列) */
