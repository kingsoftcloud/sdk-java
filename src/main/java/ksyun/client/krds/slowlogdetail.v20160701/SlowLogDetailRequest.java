...
/**
 * @Classname SlowLogDetailRequest @Description 请求参数
 */
@Data
public class SlowLogDetailRequest {
   ...
   * 排序字段
   *
   * <p>sqlExecTimeStamp：执行完成时间 currentUser：用户 hostname：客户端ip queryTime： 执行耗时 lockTime：锁等待时间
   * rowsExamined：扫描行 rowsSent：返回行
   */
  ...
  /** 多选匹配 以INSERT、UPDATE、DELETE、SELECT为首进行匹配,支持多选以|隔开，例如：HeadKey: SELECT|IN */
