...
public class CreateInstanceRequest {
  ...
  /**
   * 集群各个模块配置, 需与EnableModules参数中定义的各模块对应。Object: { "moduleType": "tidb", "packageCode": "DB_1C2G",
   * "replicas": 1, "cpu": 1, "mem": 2, "tidisk": 0 }
   */
  ...
  /**
   * 备份恢复实例ID(根据时间点恢复的源实例) ``` 指定时间点恢复，需要如下参数都传
   *
   * <p>注意： backupRestoreInstanceId backupRestoreTime ```
   */
  @KsYunField(name = "backupRestoreInstanceId")
  private String BackupRestoreInstanceId;

  /**
   * 备份恢复时间点(根据时间点恢复的指定时间点) ``` 格式：2024-03-14 10:46:02
   *
   * <p>注意： backupRestoreInstanceId backupRestoreTime ```
   */
